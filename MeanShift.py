...
USE_MEAN_SHIFT_CLUSTERING = False  # when True - clusters will be estimated from the data using MeanShift algorithm.
BANDWIDTH = 150.0
...
NUM_OF_AOI = 2  # How many Areas of Interests there are in the question. (Or, how many clusters there are) excluding
                # the outer AOI.
HARD_CODED = True  # if True you will use pre-existing clusters determined in HARD_CODED_CLUSTERS.
...
def build_cluster_jump_array_within_same_cluster_and_disable_same_cluster_in_switch_matrix(switch_mat, n_clusters_):
    ...
    plt.title(f"Subject {SUBJECT_KEY} - Question {QUESTION_IDX + 1}")
